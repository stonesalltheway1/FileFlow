...
COLORS = {
    "primary": "#2D7DD2",      # Blue
    "secondary": "#97CC04",    # Green
    "accent": "#F45D01",       # Orange
...
class FileFlowApp:
    ...
    def _setup_style(self):
        ...
        style.configure("TButton", background=primary_color, foreground="#FFFFFF", padding=5)
        ...
        style.map("TNotebook.Tab", background=[("selected", primary_color)], foreground=[("selected", "#FFFFFF")])
        
        # Configure custom styles
        style.configure("Primary.TButton", background=primary_color, foreground="#FFFFFF", font=("Arial", 10, "bold"))
        style.configure("Success.TButton", background=COLORS["success"], foreground="#FFFFFF")
        style.configure("Warning.TButton", background=COLORS["warning"], foreground="#FFFFFF")
        style.configure("Error.TButton", background=COLORS["error"], foreground="#FFFFFF")
        style.configure("Title.TLabel", font=("Arial", 14, "bold"), background=bg_color, foreground=fg_color)
        style.configure("Subtitle.TLabel", font=("Arial", 12), background=bg_color, foreground=fg_color)
        style.configure("H1.TLabel", font=("Arial", 18, "bold"), background=bg_color, foreground=primary_color)
        style.configure("H2.TLabel", font=("Arial", 16, "bold"), background=bg_color, foreground=primary_color)
        style.configure("H3.TLabel", font=("Arial", 14, "bold"), background=bg_color, foreground=fg_color)
    ...
    def _setup_dashboard(self):
        ...
        top_section = ttk.Frame(dashboard)
        top_section.pack(fill=tk.X, padx=5, pady=5)
        
        # Stats cards
        stats_frame = ttk.Frame(top_section)
        ...
        # Files scanned card
        files_card = ttk.LabelFrame(stats_frame, text="Files Scanned")
        files_card.grid(row=0, column=0, padx=5, pady=5, sticky="nesw")
        
        self.files_count_label = ttk.Label(files_card, text="0", font=("Arial", 24, "bold"))
        self.files_count_label.pack(pady=10)
        
        # Total size card
        size_card = ttk.LabelFrame(stats_frame, text="Total Size")
        size_card.grid(row=0, column=1, padx=5, pady=5, sticky="nesw")
        
        self.total_size_label = ttk.Label(size_card, text="0 MB", font=("Arial", 24, "bold"))
        self.total_size_label.pack(pady=10)
        
        # Duplicates card
        dupes_card = ttk.LabelFrame(stats_frame, text="Duplicate Files")
        dupes_card.grid(row=0, column=2, padx=5, pady=5, sticky="nesw")
        
        self.dupes_count_label = ttk.Label(dupes_card, text="0", font=("Arial", 24, "bold"))
        self.dupes_count_label.pack(pady=10)
        
        # Space saved card
        saved_card = ttk.LabelFrame(stats_frame, text="Space Wasted")
        saved_card.grid(row=0, column=3, padx=5, pady=5, sticky="nesw")
        
        self.space_saved_label = ttk.Label(saved_card, text="0 MB", font=("Arial", 24, "bold"))
        self.space_saved_label.pack(pady=10)
        
        # Middle section with recent scans and actions
        middle_section = ttk.Frame(dashboard)
        middle_section.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        middle_section.columnconfigure(0, weight=1)
        middle_section.columnconfigure(1, weight=1)
        middle_section.rowconfigure(0, weight=1)
        
        # Recent scans
        scans_frame = ttk.LabelFrame(middle_section, text="Recent Scans")
        scans_frame.grid(row=0, column=0, padx=5, pady=5, sticky="nesw")
        ...
        self.scans_tree.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        # Recent actions
        actions_frame = ttk.LabelFrame(middle_section, text="Recent Actions")
        actions_frame.grid(row=0, column=1, padx=5, pady=5, sticky="nesw")
        ...
        bottom_section = ttk.Frame(dashboard)
        bottom_section.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Label(bottom_section, text="Quick Actions", style="H3.TLabel").pack(anchor=tk.W, padx=5, pady=5)
        
        actions_container = ttk.Frame(bottom_section)
        actions_container.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Button(actions_container, text="Scan Directory", command=self._select_scan_directory).pack(side=tk.LEFT, padx=5)
        ttk.Button(actions_container, text="Find Duplicates", command=self._find_duplicates).pack(side=tk.LEFT, padx=5)
        ttk.Button(actions_container, text="Auto Organize", command=self._auto_organize).pack(side=tk.LEFT, padx=5)
        ttk.Button(actions_container, text="Cleanup Suggestions", command=self._show_cleanup_suggestions).pack(side=tk.LEFT, padx=5)
    ...
    def _show_preferences(self):
        ...
        # Theme
        ttk.Label(general_frame, text="Theme:").grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        # Max file size
        ttk.Label(scanning_frame, text="Max file size (MB):").grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)
        
        max_size_var = tk.StringVar(value=str(self.config.get_int("Scanning", "max_file_size_mb", 1000)))
        ttk.Entry(scanning_frame, textvariable=max_size_var, width=10).grid(row=3, column=1, padx=5, pady=5, sticky=tk.W)
        ...
        # Min file size
        ttk.Label(duplicates_frame, text="Min file size (KB):").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
        
        min_size_var = tk.StringVar(value=str(self.config.get_int("Duplicates", "min_size_kb", 10)))
        ttk.Entry(duplicates_frame, textvariable=min_size_var, width=10).grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)
        
        # Compare method
        ttk.Label(duplicates_frame, text="Compare method:").grid(row=2, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        compare_method_combo.grid(row=2, column=1, padx=5, pady=5, sticky=tk.W)
        
        # Hash algorithm
        ttk.Label(duplicates_frame, text="Hash algorithm:").grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        hash_algo_combo.grid(row=3, column=1, padx=5, pady=5, sticky=tk.W)
        ...
        # Default mode
        ttk.Label(organization_frame, text="Default mode:").grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        mode_combo.grid(row=1, column=1, padx=5, pady=5, sticky=tk.W)
        ...
        # Naming convention
        ttk.Label(organization_frame, text="Naming convention:").grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        naming_combo.grid(row=3, column=1, padx=5, pady=5, sticky=tk.W)
        ...
        ttk.Label(interface_frame, text="Max recent directories:").grid(row=3, column=0, padx=5, pady=5, sticky=tk.W)
        
        max_recent_var = tk.StringVar(value=str(self.config.get_int("Interface", "max_recent_dirs", 10)))
        ttk.Entry(interface_frame, textvariable=max_recent_var, width=10).grid(row=3, column=1, padx=5, pady=5, sticky=tk.W)
        
        # Buttons
        button_frame = ttk.Frame(dialog)
        button_frame.pack(fill=tk.X, pady=10)
        
        ttk.Button(button_frame, text="Save", command=lambda: self._save_preferences(
            auto_update_var.get(),
            theme_var.get(),
    ...
    def _save_preferences(self, auto_update, theme, ignore_hidden, ignore_system, max_size,
                        ...
                        tooltips, confirm, max_recent, dialog):
        """Save preferences."""
        try:
    ...
    def _update_dashboard(self):
        ...
        stats = self.duplicate_finder.get_total_duplicates_stats()
        self.dupes_count_label.config(text=str(stats.get('total_duplicates', 0)))
        self.space_saved_label.config(text=self._format_size(stats.get('wasted_size', 0)))
        
        # Update recent scans
        # (In a real implementation, this would show actual scan history)
        ...
        for dir_path, data in sorted(directories.items(), key=lambda x: x[1]['date'], reverse=True)[:10]:
            ...
            self.scans_tree.insert("", "end", values=(dir_path, count, date))
        ...
        for entry in history:
            ...
            # Format details for display
            if 'file_path' in details:
                detail_text = os.path.basename(details['file_path'])
            elif 'original_path' in details and 'new_path' in details:
                detail_text = f"{os.path.basename(details['original_path'])} -> {os.path.basename(details['new_path'])}"
            else:
                detail_text = str(details)[:50]  # Limit length
            
            # Format timestamp
            timestamp_dt = datetime.datetime.fromisoformat(timestamp) if timestamp else datetime.datetime.now()
            formatted_timestamp = timestamp_dt.strftime("%Y-%m-%d %H:%M")
            
            self.actions_tree.insert("", "end", values=(action, detail_text, formatted_timestamp))
